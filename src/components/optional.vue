<template>
	<div class="optional">
		...
		<ul class="situation">
			<li v-for="item in optional" @longtap="handleTap(item.stockId)">
				<p @longtap="handleTap(item.stockId)" class="gp_name">{{item.stockName}}<span @longtap="handleTap(item.stockId)" class="code">{{item.volume}}</span></p>
...
//	  		var stockid = sessionStorage.getItem("stockId");
	  		...
	  		ajax({
				...
				success:function(res){
					that.optional = that.optional.filter(x => x.stockId !== this.delId)
					that.delId = ''
